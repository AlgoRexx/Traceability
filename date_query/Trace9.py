...
def get_row_by_barcode(database_manager, barcode_value):
    try:
        query = f'SELECT * FROM {fixed_table_name} WHERE "Date" = ?;'
...
async def read_item(request: Request, barcode: str = Form(...)):
    current_folder = '/Users/anand/Desktop/ai/grad/date_query'
    ...
    try:
        # Parse the input barcode as a date
        parsed_date = parse_date(barcode)
        parsed_date = parsed_date.strftime('%d/%m/%y')
        csv_to_sql(csv_files, app.db_manager)

        df_row = get_row_by_barcode(app.db_manager, parsed_date)
        ...
        if not df_row.empty:
            ...
            result_data = f"No row found for Date . {barcode} or table is empty."
