...
async def read_item(request: Request, barcode: str = Form(...)):
    ...
    try:
        ...
        if not df_row.empty:
            ...
            # Open another HTML file based on a specific condition
            if int(barcode) == 1720:  # Replace YOUR_SPECIFIC_NUMBER with the desired condition
                return templates.TemplateResponse("Rresult.html", {"request": request, "result_data": result_data, "graph_data": graph_data})
